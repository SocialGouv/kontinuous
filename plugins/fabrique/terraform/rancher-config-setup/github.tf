...
resource "github_repository_webhook" "kubewebhook_repository_events_create" {
  for_each = {
    for repo in local.public_repos : repo.name => repo
  }
  ...
  configuration {
    url          = "https://webhook-${local.project_name}.fabrique.social.gouv.fr/api/v1/oas/hooks/github?event=created"
    content_type = "json"
