...
const requireUse = async (use) => {
  ...
  const downloadingPromises = ctx.require("downloadingPromises")
  const { KS_BUILD_PATH: rootDir, KS_WORKSPACE_PATH: userDir, KS_KUBEWORKFLOW_PATH } =
    ctx.require("env")
  ...
  if (!downloadingPromises[useSlug]) {
    downloadingPromises[useSlug] = (async () => {
      if (use.startsWith(".") || use.startsWith("/")) {
        ...
        const dir = path.join(KS_KUBEWORKFLOW_PATH, nativeJob)
