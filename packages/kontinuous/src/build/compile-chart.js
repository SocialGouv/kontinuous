...
module.exports = async (values) => {
  const { KS_BUILD_PATH, KS_WORKSPACE_KS_PATH } = ctx.require("env")
  
  const chart = yaml.load(
    await fs.readFile(`${KS_BUILD_PATH}/Chart.yaml`, {
      encoding: "utf-8",
    })
  )
  ...
  const allChartNames = [...(new Set(
    (await Promise.all([
      getDirectories(`${KS_BUILD_PATH}/charts`),
      getDirectories(`${KS_WORKSPACE_KS_PATH}/charts`),
    ])).reduce((acc, dirNames) => [...acc, ...dirNames], [])
  ))]

  for (const c of allChartNames){
    
    const chartDir = `${KS_BUILD_PATH}/charts/${c}`
  ...
  await fs.writeFile(`${KS_BUILD_PATH}/Chart.yaml`, yaml.dump(chart))
